...
const BCRYPT_SALT_ROUNDS = parseInt(
  process.env.BCRYPT_SALT_ROUNDS || process.env.BCRYPT_ROUNDS || "12",
  10
);
...
async function main() {
  ...
  const csvPath = process.argv[2] || path.join(__dirname, "../data/routes_dataset.csv");
  ...
  for (const r of resCarriers.rows) carrierCache.set(r.name, r.id);
  ...
  for await (const record of parser as AsyncIterable<CsvRow>) {
    ...
    try {
      ...
      const distance = parseFloat(
        readAlias(record, ["distance_km", "distance km"], "0")
      );
      const estimated = parseFloat(
        readAlias(record, ["estimated_time_hours", "estimated time_hours"], "0")
      );
      ...
      if (!origin || !destination) throw new Error("origin or destination missing");
      if (isNaN(distance) || distance <= 0) throw new Error("invalid distance_km");
      if (isNaN(estimated) || estimated <= 0) throw new Error("invalid estimated_time_hours");
      if (!vehicle) throw new Error("vehicle_type missing");
      if (!carrierName) throw new Error("carrier missing");
      if (isNaN(cost) || cost < 0) throw new Error("invalid cost_usd");
